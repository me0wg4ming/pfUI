pfUI:RegisterModule("gui", "vanilla:tbc", function ()
  ...
  do -- Generate Config UI
    ...
    CreateGUIEntry(T["Unit Frames"], T["General"], function()
      ...
      CreateConfig(nil, T["Show Druid Mana Bar Text"], C.unitframes, "druidmanatext", "checkbox", nil, nil, nil, nil, "vanilla" )
    ...
    CreateGUIEntry(T["Buffs"], T["Buff/Debuff Icons"], function()
      CreateConfig(nil, T["Enable Buff Display"], C.buffs, "buffs", "checkbox")
      CreateConfig(nil, T["Enable Debuff Display"], C.buffs, "debuffs", "checkbox")
    ...
    for _, data in pairs(barnames) do
      ...
      local uneven_options = function()
        ...
        if cols == 3 and rows == 3 then return { "Up", "Down", "Left", "Right" }
        elseif cols and cols <= 3 then return { "Up", "Down" }
        elseif rows and rows <= 3 then return { "Left", "Right" }
        end
      ...
      CreateGUIEntry(T["Actionbar"], caption, function()
        ...
        local layout_ufunc = function()
          -- reset uneven when formfactor changes if current value is invalid for new shape
          local formfactor = tostring(C.bars["bar"..id].formfactor or "")
          local _, _, cols, rows = string.find(formfactor, "(%d+)%s*x%s*(%d+)")
          cols, rows = tonumber(cols), tonumber(rows)
          ...
          -- refresh uneven dropdown to show new valid options and current value
